...
class AirqualityValidationException implements Exception {
  final List<String> errors;

  AirqualityValidationException(this.errors);

  @override
  String toString() => 'AirqualityValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://airquality.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [city] (required) - The city name for which you want to get the air quality data (e.g., New York)
class AirqualityClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'city': ValidationRule(type: 'string', required: true),
  };

  AirqualityClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/airquality',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AirqualityValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AirqualityValidationException(errors);
    }
  }

  /// Execute a request to the Air Quality API.
  ///
  /// Parameters are validated before sending the request.
  Future<AirqualityResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AirqualityException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AirqualityException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AirqualityException('Resource not found');
      } else {
        throw AirqualityException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AirqualityException) rethrow;
      throw AirqualityException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Air Quality API client.
class AirqualityException implements Exception {
  final String message;

  AirqualityException(this.message);

  @override
  String toString() => 'AirqualityException: $message';
}
